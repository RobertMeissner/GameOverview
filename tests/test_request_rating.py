...
class TestRequestRating(unittest.TestCase):
        ...
        mock_request.side_effect = [
            Mock(status_code=200, text=json.dumps({
                "hits": [{
                    ...
                    "_highlightResult": {
                        "name": {
                            "value": "<strong>Cyberpunk</strong> 2077"
                        }
                    }
                }]
            })),
    ...
    def test_rating_api_steam(self):
        ...
        self.assertIn("_highlightResult", data["hits"][0], "First hit does not contain '_highlightResult'")
        self.assertIn("name", data["hits"][0]["_highlightResult"], "Highlight results do not contain 'name'")
        self.assertIn("value", data["hits"][0]["_highlightResult"]["name"], "name key does not contain 'value'")
