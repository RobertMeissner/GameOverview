...
def parse_steam_family_html_for_gamelist(file_path: str) -> pd.DataFrame:
    ...
    def extract_game_data(html_soup):
        games = []
        for img in html_soup.find_all("img", class_="_3dBfx1sV1COVdSsh7RdgMd"):  # Adjust class if needed
    ...
    new_rows = []
    for game in data:
        new_rows.append({game_name: game[game_name], store_name: "steam", played_flag: False})
...
def steam_family_games():
    return parse_steam_family_html_for_gamelist(DATA_FOLDER + "/steam_family_account.html")
...
if __name__ == "__main__":
    ...
    df = parse_steam_family_html_for_gamelist(DATA_FOLDER + "/steam_family_account.html")
    print(df.head())
