...
def load_input_game_names():
    if CONFIG["inputFile"]["fileType"] not in ["csv", "txt"]:
        print(f"Error: Input file type not supported: {CONFIG['inputFile']['fileType']}.")
        exit(1)

    try:
        with open(f"{CONFIG['inputFile']['fileName']}.{CONFIG['inputFile']['fileType']}") as file:
...
def steam_app_ids_from_game_names():
    ...
    if len(steam_ids_single_full_match) > 0:
        ...
        with open(f"./output/{CONFIG['mode']}/steamAppIds_fullMatches.json", "w") as file:
    ...
    if len(steam_ids_multiple_full_matches) > 0:
        ...
        with open(f"./output/{CONFIG['mode']}/steamAppIds_multipleFullMatches.json", "w") as file:
            json.dump(steam_ids_multiple_full_matches, file, indent=2)
    print()

    if not CONFIG["onlyFullMatches"]:
        # Find Steam App ID's for best matches
        steam_ids_best_match, steam_ids_no_match = find_steam_app_ids_best_match(game_names, steam_apps)

        # Save the best matches to a .json file
        print(
            f"\nWriting game names and Steam App ID's for partial matches to" f" \"output/{CONFIG['mode']}/steamAppIds_bestMatch.json\"..."
        )
        ...
        if len(steam_ids_no_match) > 0:
            ...
            with open(f"./output/{CONFIG['mode']}/steamAppIds_noMatch.json", "w") as file:
...
def find_steam_app_ids_full_match(game_names, steam_apps):
    ...
    for game in game_names:
        ...
        full_matches = list({app["appid"] for app in steam_apps if app["name"] == game})
...
def find_steam_app_ids_best_match(game_names, steam_apps):
    ...
    for i in tqdm(range(len(game_names_lowercase))):
        ...
        if best_match:
            steam_ids_best_match[game_names[i]] = {
                ...
                "similarity": difflib.SequenceMatcher(None, game_names_lowercase[i], best_match[0]).ratio(),
                "steamName": steam_apps[steam_apps_lowercase.index(best_match[0])]["name"],
            }
        else:
    ...
    print(f"Found partial matches with a similarity score >={partial_match_threshold} for " f"{len(steam_ids_best_match)} games.")
