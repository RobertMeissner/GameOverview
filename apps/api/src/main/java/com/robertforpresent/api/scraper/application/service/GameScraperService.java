...
/**
 * Service for searching and scraping game information from IGDB.
 */
@Service
@Slf4j
public class GameScraperService {
    ...
    public GameScraperService(IgdbConfig config, IgdbAuthService authService) {
        this.config = config;
    ...
    /**
     * Searches for games by name on IGDB.
     *
    ...
    public GameSearchResult searchGames(String query, int limit) {
        if (!config.isEnabled()) {
            ...
            return new GameSearchResult(query, List.of(), "igdb");
        }
        ...
        try {
            List<ScrapedGameInfo> results = searchIgdb(query, limit, tokenOpt.get());
            return new GameSearchResult(query, results, "igdb");
        } catch (Exception e) {
    ...
    /**
     * Gets detailed information for a specific game by IGDB ID.
     */
    public Optional<ScrapedGameInfo> getGameDetails(long igdbId) {
        if (!config.isEnabled()) {
        ...
        try {
            ...
            log.error("Error fetching game details for ID {}", igdbId, e);
        }
